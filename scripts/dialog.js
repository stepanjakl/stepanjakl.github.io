...
// eslint-disable-next-line no-redeclare -- Local reference for convenience
...
aria.getEl = getElementByIdWithCaching;
 ...
 */
function registerLifecycleHooks(dialogId, hooks) {
	aria.lifecycleHooks[dialogId] = hooks;
}
aria.registerLifecycleHooks = registerLifecycleHooks;
 ...
 */
function callLifecycleHook(dialogId, hookType) {
	const hooks = aria.lifecycleHooks[dialogId];
	if (hooks && typeof hooks[hookType] === 'function') {
		hooks[hookType]();
	}
}
aria.callLifecycleHook = callLifecycleHook;
...
aria.getCurrentDialog = getCurrentDialog;
 ...
 */
function closeCurrentDialog(hash) {
...
aria.closeCurrentDialog = closeCurrentDialog;
...
aria.Dialog = function (dialogId, focusAfterClosed, focusFirst, hash) {
	...
	this.inertNode = inertTargetId ? aria.getEl(inertTargetId) : this.dialogNode;
...
aria.Dialog.prototype.setupFocusElements = function (focusAfterClosed, focusFirst) {
	...
	this.focusAfterClosed =
		typeof focusAfterClosed === 'string' ? aria.getEl(focusAfterClosed) : focusAfterClosed;
	...
	this.focusFirst = typeof focusFirst === 'string' ? aria.getEl(focusFirst) : focusFirst || null;

	// Validate focusFirst has a focus method if it's not null
	if (this.focusFirst && typeof this.focusFirst.focus !== 'function') {
		console.warn(
			'aria.Dialog: focusFirst element does not have a focus() method, will use default focus behaviour',
			this.focusFirst
		);
 ...
 */
function ariaOpenDialog(dialogId, focusAfterClosed, focusFirst, hash) {
	if (aria.OpenDialogList.length > 0) {
...
window.openDialog = ariaOpenDialog;
...
window.closeDialog = ariaCloseDialog;
